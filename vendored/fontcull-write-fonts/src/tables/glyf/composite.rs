...
use fontcull_read_fonts::{tables::glyf::CompositeGlyphFlags, types::GlyphId16, FontRead};

use super::Bbox;

pub use fontcull_read_fonts::tables::glyf::{Anchor, Transform};
...
impl FromObjRef<fontcull_read_fonts::tables::glyf::CompositeGlyph<'_>> for CompositeGlyph {
    fn from_obj_ref(
        from: &fontcull_read_fonts::tables::glyf::CompositeGlyph,
        _data: fontcull_read_fonts::FontData,
    ) -> Self {
        let bbox = Bbox {
            x_min: from.x_min(),
            y_min: from.y_min(),
